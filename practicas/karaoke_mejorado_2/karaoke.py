...
def efecto_escribir_centrado(texto, color, tiempo_espera=0.05):
    lineas = texto.split('\n')
    tiempo_por_linea = tiempo_espera / len(lineas) if len(lineas) > 0 else tiempo_espera
    ...
    for linea in lineas:
        ...
        tiempo_por_letra = tiempo_por_linea / len(linea)
        espacio_izquierdo = " " * ((ancho_terminal - len(linea)) // 2)
        
        sys.stdout.write(espacio_izquierdo + color + NEGRITA)
...
def reproducir_karaoke():
    ...
    print(f"{AMARILLO}{NEGRITA}{titulo.center(ancho_terminal)}{RESET}\n")
    time.sleep(3)
    ...
    if AUDIO_DISPONIBLE:
        try:
            ...
            pygame.mixer.music.load('practicas/karaoke_mejorado_2/pista.mp3')
            pygame.mixer.music.play()
        except Exception as e:
            mensaje_error = f"Error al reproducir el audio: {e}"
            print(f"{AMARILLO}{NEGRITA}{mensaje_error.center(ancho_terminal)}{RESET}")
            time.sleep(2)
    ...
    while not pila_karaoke.esta_vacia():
        ...
        efecto_escribir_centrado(texto, color, espera)
